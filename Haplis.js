...
var canvas, ctx;
var score, timer, gameTick, multiplier;
var gameboard;
var currentPiece = null;
var keyLeft = false, keyRight = false, keyDown = false, fastFall = false, isBuffered = null, autoPlay = false, smartAI = false, canSwap = true;
const width = 10, height = 20, size = 32, fullWidth = (10 + width) * size, fullHeight = (1 + height) * size;
var haplominoes = [];
var bg;
var nextPieces = [], holdPiece;
function OnLoad() {
...
document.onkeydown = function EnterKey(e) {
    ...
    if (e.code == "KeyA" || e.code == "KeyJ" || e.code == "ArrowLeft") {
        ...
        holdPiece = tempColor;
        if (currentPiece.c == null)
            currentPiece = GetNextPiece();
    }
    else if (e.code == "KeyS" || e.code == "KeyK" || e.code == "ArrowDown")
        keyDown = true;
    else if (e.code == "KeyW" || e.code == "KeyI" || e.code == "ArrowUp")
        fastFall = true;
...
function MoveUpdate() {
    ...
    if (currentPiece != null) {
        if (smartAI) {
            ...
            let first = gameboard[0].indexOf(0), last = gameboard[0].lastIndexOf(0), x;
            if (first == last)
                x = -1;
            else if (first <= width - last - 1 && first != 4)
                x = first;
            else
                x = last;
            if (isBuffered != null && gameboard[height - 1].filter((e) => e).length < 0)
                isBuffered = null;
            if (x == -1 || isBuffered != null) {
                fastFall = true;
                isBuffered = true;
            }
            else if (x - currentPiece.x > 0)
                keyRight = true;
            else if (x - currentPiece.x < 0)
                keyLeft = true;
            else
        ...
        if (keyLeft && currentPiece.x != 0 && !gameboard[currentPiece.y][currentPiece.x - 1]) {
            ...
            else if (isBuffered != null && isBuffered)
                isBuffered = false;
            Draw();
        }
...
function Draw() {
    ...
    for (var i = 0; i < height; i++)
        for (var j = 0; j < width; j++) {
            if (gameboard[i][j])
                ctx.putImageData(haplominoes[gameboard[i][j]], (j + 5) * size, i * size);
        }
    if (currentPiece != null)
        ctx.putImageData(haplominoes[currentPiece.c], (currentPiece.x + 5) * size, currentPiece.y * size);
    for (let i = 0; i < nextPieces.length; i++)
        if (nextPieces[i] != null)
            ctx.putImageData(haplominoes[nextPieces[i]], (width + 7) * size, (i * 2 + 7) * size);
    if (holdPiece != null)
        ctx.putImageData(haplominoes[holdPiece], 2 * size, 5 * size);
    ctx.fillStyle = "#FFF";
    ctx.font = "30px Arial";
...
function GameOver() {
    ...
    ctx.fillText("High Score: " + highScore, ((width + 10) / 2) * size, -2.25 * size, (width - 2) * size);
    console.log(highScore);
	timer = null;
}
function GetHighScore(){
	if (window.localStorage) {
		var highScore = JSON.parse(window.localStorage.getItem("haplis-highscore"));
...
function GenerateImageData() {
    return __awaiter(this, void 0, void 0, function* () {
        function GenerateHaplomino(base, light, dark) {
            ctx.fillStyle = base;
        ...
        haplominoes = [
            GenerateHaplomino("#303030", "#505050", "#101010"),
            GenerateHaplomino("#DEDE30", "#FFFF50", "#4A4A10"),
            GenerateHaplomino("#30DEDE", "#50FFFF", "#4A4A10"),
            GenerateHaplomino("#30DE30", "#50FF50", "#104A4A"),
            GenerateHaplomino("#DE30DE", "#FF50FF", "#4A104A"),
            GenerateHaplomino("#DE3030", "#FF5050", "#4A1010"),
            GenerateHaplomino("#3030DE", "#5050FF", "#10104A"),
            GenerateHaplomino("#FF6A30", "#FF8A50", "#DE4A10"),
            GenerateHaplomino("#DEDEDE", "#FFFFFF", "#4A4A4A"),
            GenerateHaplomino("#4E4E4E", "#838383", "#1A1A1A"), // L.Gray - 9
        ];
        let haplominoBg = createImageBitmap(haplominoes[9]);
        ctx.fillStyle = ctx.createPattern(yield haplominoBg, "repeat");
...
function RandomColor() {
    ...
    for (var i = 0; i < 6; i++) {
        ...
        if (nextPieces.findIndex((e) => e == selectedColor) == -1)
            break;
    }
    return selectedColor;
}
function PlayMusic() {
    var p = document.getElementById("jams").play();
    p.then(() => { }).catch((e) => { });
}
//# sourceMappingURL=Haplis.js.map